...
# Load each city's SVR model
models = {}
for city in cities:
    model = joblib.load(f'models/svr_lr/{city}_svr_model.pkl')  # Load SVR models
    models[city] = model
...
for city in cities:
    ...
    # Predict the temperatures for this city using the respective SVR model
    city_predictions = models[city].predict(X.values)
...
print(f"Weighted Averaging Model MSE (SVR): {weighted_mse}")

# Save the weighted predictions (this is for reference)
joblib.dump(weighted_predictions, 'models/combines/weighted_averaging_model_svr.pkl')

print("Weighted Averaging model predictions (SVR) saved successfully!")
